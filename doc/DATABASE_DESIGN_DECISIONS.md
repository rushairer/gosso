...
**⚠️ 答案：选择性使用数据库外键约束，结合应用层约束**

#### 理由对比

| 维度 | 数据库外键 | 应用层约束 | 本项目选择 |
|------|-----------|-----------|-----------|
| **数据一致性** | ✅ 强制保证 | ⚠️ 需代码保证 | 混合模式（核心表用外键） |
| **性能** | ❌ INSERT/UPDATE/DELETE 时锁竞争 | ✅ 无额外开销 | 关键路径优化 |
| **扩展性** | ❌ 无法跨库分片 | ✅ 易于分库分表 | 核心单体库适用 |
| **部署灵活性** | ❌ 迁移困难 | ✅ 灵活调整 | 结合使用 |
| **死锁风险** | ❌ 高（多表关联） | ✅ 低 | 控制级联深度 |

#### 本项目的外键策略

本项目在核心数据一致性、级联删除（如账号删除时级联删除凭证、会话、OAuth2 客户端与授权同意书）等强一致性场景中，**保留并显式声明了数据库外键约束**，并配合 `ON DELETE CASCADE`。而在高并发、非核心关联或跨服务边界的场景下，则倾向于使用应用层约束。
...
2. **选择性外键** - 核心表使用外键加级联删除保证强一致性，非核心表使用应用层约束
...
- [ ] 仅在非核心或高并发场景移除 FOREIGN KEY 约束，核心表保留级联删除外键
- [ ] 所有修改操作使用事务包装
- [ ] 实施定期数据一致性校验
- [ ] 添加数据不一致告警监控
- [ ] 编写事务失败重试逻辑
