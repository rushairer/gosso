...
func (r *RedisClient) Set(ctx context.Context, key string, value any, expiration time.Duration) error {
	...
	if err != nil {
		r.logger.Debug("Redis SET failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) Get(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		r.logger.Debug("Redis GET failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) Del(ctx context.Context, keys ...string) error {
	...
	if err != nil {
		r.logger.Debug("Redis DEL failed", zap.Strings("keys", maskRedisKeySuffixes(keys)), zap.Error(err))
...
func (r *RedisClient) Exists(ctx context.Context, key string) (bool, error) {
	...
	if err != nil {
		r.logger.Debug("Redis EXISTS failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) Expire(ctx context.Context, key string, expiration time.Duration) error {
	...
	if err != nil {
		r.logger.Debug("Redis EXPIRE failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) Incr(ctx context.Context, key string) (int64, error) {
	...
	if err != nil {
		r.logger.Debug("Redis INCR failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) Decr(ctx context.Context, key string) (int64, error) {
	...
	if err != nil {
		r.logger.Debug("Redis DECR failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) SetNX(ctx context.Context, key string, value any, expiration time.Duration) (bool, error) {
	...
	if err != nil {
		r.logger.Debug("Redis SETNX failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) HSet(ctx context.Context, key string, values ...any) error {
	...
	if err != nil {
		r.logger.Debug("Redis HSET failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) HGet(ctx context.Context, key, field string) (string, error) {
	...
	if err != nil {
		r.logger.Debug("Redis HGET failed", zap.String("key", maskRedisKeySuffix(key)), zap.String("field", field), zap.Error(err))
...
func (r *RedisClient) HGetAll(ctx context.Context, key string) (map[string]string, error) {
	...
	if err != nil {
		r.logger.Debug("Redis HGETALL failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) HDel(ctx context.Context, key string, fields ...string) error {
	...
	if err != nil {
		r.logger.Debug("Redis HDEL failed", zap.String("key", maskRedisKeySuffix(key)), zap.Strings("fields", fields), zap.Error(err))
...
func (r *RedisClient) SAdd(ctx context.Context, key string, members ...any) error {
	...
	if err != nil {
		r.logger.Debug("Redis SADD failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) SAddWithTTL(ctx context.Context, key string, member string, ttl time.Duration) error {
	...
	if err != nil {
		r.logger.Debug("Redis SADD+EXPIRE failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) SMembers(ctx context.Context, key string) ([]string, error) {
	...
	if err != nil {
		r.logger.Debug("Redis SMEMBERS failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) SIsMember(ctx context.Context, key string, member any) (bool, error) {
	...
	if err != nil {
		r.logger.Debug("Redis SISMEMBER failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) SRem(ctx context.Context, key string, members ...any) error {
	...
	if err != nil {
		r.logger.Debug("Redis SREM failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) TTL(ctx context.Context, key string) (time.Duration, error) {
	...
	if err != nil {
		r.logger.Debug("Redis TTL failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) IncrWithExpiry(ctx context.Context, key string, expiry time.Duration) (int64, error) {
	...
	if err != nil {
		r.logger.Debug("Redis IncrWithExpiry failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) CheckAndIncr(ctx context.Context, key string, limit int, expiry time.Duration) (int64, error) {
	...
	if err != nil {
		r.logger.Debug("Redis CheckAndIncr failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) SetIfExists(ctx context.Context, key string, value any, expiry time.Duration) (bool, error) {
	...
	if err != nil {
		r.logger.Debug("Redis SetIfExists failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
...
func (r *RedisClient) GetDel(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		r.logger.Debug("Redis GETDEL failed", zap.String("key", maskRedisKeySuffix(key)), zap.Error(err))
		return "", fmt.Errorf("getDel: %w", err)
	}
	return result, nil
}

// maskRedisKeySuffix redacts the last segment of a Redis key (after the last colon)
// to prevent PII such as email addresses or IP addresses from appearing in logs.
// Only the final colon-separated segment is masked to preserve key-prefix context
// (e.g. namespace, operation) while hiding the PII-bearing suffix.
// Example: "verify:code:email:user@example.com" → "verify:code:email:***"
func maskRedisKeySuffix(key string) string {
...
// maskRedisKeySuffixes applies maskRedisKeySuffix to a slice of keys.
func maskRedisKeySuffixes(keys []string) []string {
	masked := make([]string, len(keys))
	for i, k := range keys {
		masked[i] = maskRedisKeySuffix(k)
	}
	return masked
}
...
func (r *RedisClient) ScanKeys(ctx context.Context, cursor uint64, pattern string, count int64) ([]string, uint64, error) {
	...
	if err != nil {
		r.logger.Debug("Redis SCAN failed", zap.String("pattern_masked", maskRedisKeySuffix(pattern)), zap.Error(err))
		return nil, 0, fmt.Errorf("scan pattern %s: %w", maskRedisKeySuffix(pattern), err)
	}
